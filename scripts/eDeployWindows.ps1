...
function Show-Banner {
    Write-Host "========================================="
    Write-Host "=== Folder Customizer Deployment Tool ==="
    Write-Host "========================================="
    Write-Host ""
...
function Build-Project {
    ...
    try {        
        ...
        # Use build/build-windows structure as requested
        $buildBaseDir = Join-Path $projectRoot "build"
        if (-not (Test-Path $buildBaseDir)) {
            New-Item -ItemType Directory -Path $buildBaseDir -Force | Out-Null
        }
        $buildDir = Join-Path $buildBaseDir "build-windows"
        ...
        # Configure with CMake - Set up MinGW environment
        $cmakeArgs = @()
        $cmakeArgs += "-DCMAKE_BUILD_TYPE=$Configuration"
        
        # Set MinGW toolchain
        $cmakeArgs += "-DCMAKE_C_COMPILER=C:/msys64/mingw64/bin/gcc.exe"
        $cmakeArgs += "-DCMAKE_CXX_COMPILER=C:/msys64/mingw64/bin/g++.exe"
        $cmakeArgs += "-DCMAKE_RC_COMPILER=C:/msys64/mingw64/bin/windres.exe"
        ...
        if (-not [string]::IsNullOrEmpty($QtPath)) {
            ...
            # Hardcoded Qt path for MinGW
            $defaultQtPath = "C:/Qt/6.9.2/mingw_64"
            Write-Host "Using default Qt path: $defaultQtPath"
            $cmakeArgs += "-DCMAKE_PREFIX_PATH=`"$defaultQtPath`""
        }
        ...
        $sourcePath = $projectRoot  # CMakeLists.txt is in the root directory
        $cmakeCmd = "cmake -G `"$CMakeGenerator`" $($cmakeArgs -join ' ') `"$sourcePath`""
...
function Install-Project {
    ...
    try {
        $buildDir = Join-Path (Join-Path $projectRoot "build") "build-windows"
        Push-Location $buildDir
        
        # Run CMake install
        Write-Host "Installing with CMake..."
        cmake --install . --config $Configuration --prefix (Join-Path $projectRoot "dist" | Join-Path -ChildPath "windows")
        ...
        if (Get-Location -ErrorAction SilentlyContinue | Where-Object { $_.Path -like "*build" }) {
            Pop-Location
        }
        exit 1
    }
...
function Pack-WithInnoSetup {
    ...
    try {
        ...
        # Define output directory for installer
