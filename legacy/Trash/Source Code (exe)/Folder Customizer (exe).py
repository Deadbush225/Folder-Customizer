	...
	def install_key(self):
		...
		for tone in ["Light", "Normal", "Dark"]:
			...
			for color in ["Red", "Brown", "Orange", "Lemon", "Green", "Azure", "Blue", "Pink", "Violet", "White", "Gray", "Black"]:
				...
				reg.SetValue(key3, '', reg.REG_SZ, f'"{python_exe}" "{program_path}\\Folder Customizer.exe" "%V" "{tone}" "{color}"') # using python
			...
			reg.SetValue(key5, '', reg.REG_SZ, f'"{python_exe}" "{program_path}\\Folder Customizer.exe" "%V" "Reset" "Reset"')
