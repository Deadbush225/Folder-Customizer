...
def folderColorizerTagger(path, folder, tone, color, tag, default=False):
	...
	setIcon(folder, str(icon_path), 0, default=default)
	...
	if path.exists():
		...
		with open(path, "r", encoding="utf-8") as file:
			...
			if (contents.isspace()) or (contents == ''):
				...
				# os.remove(path)
				print(path.resolve())
				subprocess.run([r"C:\Program Files\Folder Customizer\DeskEdit.exe", fr"/F={str(path.parent)}", r"/D"])
